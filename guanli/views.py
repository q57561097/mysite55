...
class Token():
    ...
    def confirm_validate_token(self, token, expiration=36):
        serializer = utsr(self.security_key)
    ...
    if request.method =='POST':
        ...
        if uf.is_valid():
            ...
            if user:
                ...
                info='验证码错误'
            return render(request, 'guanli/regist.html', {'uf': uf, 'info': info})
    else:
        uf=RegistForm()
    return render(request,'guanli/regist.html',{'uf':uf,})
def login(request):
    if request.method=='POST':
        ...
        if uf.is_valid():
            ...
            if user:
                ...
                info='用户名不存在'
            return render(request, 'guanli/login.html', {'uf': uf,'info':info})

    else:
            uf=UserForm()
    return render(request,'guanli/login.html', {'uf': uf})
...
def changepass(request):
    if request.method == 'POST':
        ...
        uf = ChangeForm()
    return render(request,'guanli/change.html', {'uf': uf})
    ...
    try:
        ...
        if user:
           ...
           if user.is_active==True:
            ...
            message='账户不存在，请重新注册'
        return render(request, 'guanli/message.html', {'message': message})
    try:
        user = User.objects.get(username=username)
